...
const cardImages = [
  { src: 'https://picsum.photos/seed/jaehyun1/200/200', id: 1, hint: 'kpop idol smile' },
  { src: 'https://picsum.photos/seed/jaehyun2/200/200', id: 2, hint: 'kpop idol stage' },
  { src: 'https://picsum.photos/seed/jaehyun3/200/200', id: 3, hint: 'kpop idol casual' },
  { src: 'https://picsum.photos/seed/jaehyun4/200/200', id: 4, hint: 'kpop idol suit' },
  { src: 'https://picsum.photos/seed/jaehyun5/200/200', id: 5, hint: 'kpop idol funny' },
  { src: 'https://picsum.photos/seed/jaehyun6/200/200', id: 6, hint: 'kpop idol glasses' },
  { src: 'https://picsum.photos/seed/jaehyun7/200/200', id: 7, hint: 'kpop idol cute' },
  { src: 'https://picsum.photos/seed/jaehyun8/200/200', id: 8, hint: 'kpop idol serious' },
];
...
const GameCard = ({ card, handleCardClick }: { card: CardType, handleCardClick: (card: CardType) => void }) => {
  return (
    <div className="perspective-1000" onClick={() => !card.isFlipped && !card.isMatched && handleCardClick(card)}>
      <div className={`relative w-full h-full preserve-3d transition-transform duration-500 ${card.isFlipped || card.isMatched ? 'rotate-y-180' : ''}`}>
        ...
        <div className="absolute w-full h-full backface-hidden flex items-center justify-center bg-primary/20 rounded-lg border-2 border-primary/50 cursor-pointer hover:bg-primary/40">
          <span className="text-4xl" style={{textShadow: '0 0 10px hsl(var(--primary))'}}>🍑</span>
        </div>
        {/* Card Front */}
  ...
  useEffect(() => {
    if (flippedCards.length === 2) {
      ...
      if (firstCard.id === secondCard.id) {
        ...
        setTimeout(() => {
          const newCards = cards.map(card => 
            card.uniqueId === firstCard.uniqueId || card.uniqueId === secondCard.uniqueId ? { ...card, isFlipped: false } : card
          );
          setCards(newCards);
          setFlippedCards([]);
        }, 1200);
    ...
    <div className="w-full max-w-2xl mx-auto">
        ...
        {isGameWon ? (
             <GlassCard className="p-8 text-center flex flex-col items-center">
                 ...
                 <p className="text-muted-foreground mb-4">You matched all the cards in {moves} moves.</p>
